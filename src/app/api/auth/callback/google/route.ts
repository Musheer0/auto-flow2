...
export const GET = async(req:NextRequest)=>{
    ...
    if(!data) return NextResponse.json({error:"error getting user info"}, {status:400})
    const isNewUser =!! tokens.refresh_token
    let user:user|null = null
    if(!data.email && !data.name ) return NextResponse.json({error:"missing required data"},{status:400})
    if(isNewUser){
       try {
